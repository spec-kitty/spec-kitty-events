...
55 rows total: 14 journal rows + 16 volatile rows (the Ephemeral Team
Status vocabulary — ``WPStatusChanged``, ``MissionCreated``,
``MissionClosed``, ``PhaseEntered``, the six mission-run types — plus 6
observation rows keyed by ``(event_type="HarnessObservation", kind)``,
each carrying its own payload ID -- exactly six payload IDs, matching
``HARNESS_OBSERVATION_PAYLOAD_IDS``) + 25 durable live-work rows keyed by
``(event_type="WorkObservation", kind)`` (spec-kitty-events#55), each
carrying its own payload ID, matching ``WORK_OBSERVATION_PAYLOAD_IDS``.
"""
...
EXPECTED_ROW_KEYS: frozenset[tuple[str, str | None]] = frozenset(
    {
        ...
        # Durable live-work rows (spec-kitty-events#55): one per WorkKind.
        *{("WorkObservation", kind.value) for kind in WorkKind},
    }
)
...
def test_support_matrix_has_exactly_55_rows() -> None:
    assert len(SUPPORT_MATRIX) == 55
...
def test_payload_ids_match_their_families() -> None:
    """Six harness payload IDs and 25 work payload IDs, each matching its
    module's closed set exactly (the #55 analogue of the six-ID pin)."""
    payload_ids = {row.payload_id for row in SUPPORT_MATRIX if row.payload_id is not None}
    assert payload_ids == EXPECTED_PAYLOAD_IDS | WORK_OBSERVATION_PAYLOAD_IDS
    assert len(payload_ids) == 6 + len(WorkKind)
    assert payload_ids >= HARNESS_OBSERVATION_PAYLOAD_IDS
    assert WORK_OBSERVATION_PAYLOAD_IDS <= payload_ids


def test_journal_rows_have_no_payload_id() -> None:
    for row in SUPPORT_MATRIX:
        if row.event_type not in ("HarnessObservation", "WorkObservation"):
...
def test_support_matrix_json_has_55_entries() -> None:
...
# ---------------------------------------------------------------------------
# Durable live-work rows (spec-kitty-events#55, planning#2268)
# ---------------------------------------------------------------------------


def test_work_rows_are_durable_and_complete() -> None:
    """Every WorkKind has exactly one durable support-matrix row, with the
    payload ID the module derives for it — the machine-readable answer to
    "which durable observations can this package validate" that the
    ingestion (saas#1814), relay (zeitgeist#304) and projection (saas#1816)
    consumers pin against."""
    work_rows = [row for row in SUPPORT_MATRIX if row.event_type == "WorkObservation"]
    assert len(work_rows) == len(WorkKind) == 25
    for row in work_rows:
        assert row.durability == "durable", row.kind
        assert row.family == "work", row.kind
        assert row.payload_id == f"work.{row.kind}.v1", row.kind
        assert row.model == "spec_kitty_events.work_observation.WorkObservationPayload"
    assert {row.kind for row in work_rows} == {kind.value for kind in WorkKind}
    assert {row.payload_id for row in work_rows} == WORK_OBSERVATION_PAYLOAD_IDS


def test_work_rows_pin_one_package_floor() -> None:
    """All 25 rows publish one min_consumer_package/strict_since floor —
    the 10.1.0 that first carries the contract."""
    work_rows = [row for row in SUPPORT_MATRIX if row.event_type == "WorkObservation"]
    for row in work_rows:
        assert row.introduced_in == "10.1.0", row.kind
        assert row.strict_since == "10.1.0", row.kind
        assert row.min_consumer_package == "10.1.0", row.kind


def test_work_observation_is_admitted_to_the_strict_profile() -> None:
    assert "WorkObservation" in STRICT_EVENT_TYPES
